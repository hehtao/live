 ...
 *
 *  @return YES  支持屏幕旋转,NO  不支持
 *  Example: 如果返回的是(interfaceOrientation == UIInterfaceOrientationPortrait)表示只支持home键在最下面的那个方向
 *           ((interfaceOrientation == UIInterfaceOrientation) || (interfaceOrientation ==        
 *           UIInterfaceOrientationLandscapeLeft))表示支持home键在下面和在左边的两个方向，以此类推。。。
 */
